...
def message_counter(update,context):
    ...
    if chat_type == "group" or chat_type == "supergroup":
        ...
        if Ranking.check_ranking_status(user_id=user.id):
            ...
            print(totalpts)
            print((user_messages >=followermsg and user_messages < apprenticemsg ))
            print((totalpts>=followerpts and totalpts < apprenticepts))
            if (user_messages >=followermsg and user_messages < apprenticemsg ) and (totalpts>=followerpts and totalpts < apprenticepts):
                ...
                # context.bot.send_message(chat_id=group_id, text=texts.ADVANCE.format(username,rank), reply_markup=main_markup)
