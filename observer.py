    ...
    if Ranking.check_ranking_status(user_id=user.id):
        if user_answer in tickers:
            ...
            if Users.check_user(user_id=user.id) == False:
                Users(user_id=user.id, username=util.get_username(update,context), role=0).save()
            if check_player:
                    ...
                    if game_stat == 0:
                        if Sherlockchance.get_user_chance(user_id=user.id,group_id=group_id)!="False":
                            ...
                            if int(chances) >= 2:
                                ...
                                if answ == False:
                                    Answers(user_id=user.id,username=util.get_username(update,context), correct=0, incorrect=0, answertype="sherlock",group_id=group_id).save()
                ...
                update.message.reply_text("You did not join the hunt,either join the hunt or keep calm and wait for the next hunt")
            ...
            if bot_username == config.BOT_USERNAME and (chat_type == "group" or chat_type == "supergroup"):
                ...
                if qstn_type == 'apolo':
                    ...
                    if answ == False:
                        Answers(user_id=user.id,username=util.get_username(update,context), correct=0, incorrect=0, answertype=qstn_type,group_id=group_id).save()
        ...
        update.message.reply_text(f"Hi {user.first_name}, please open @{context.bot.username} send /join to register for the points program")
