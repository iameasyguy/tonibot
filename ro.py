...
def activity_select(update, context):
    ...
    #check user role
    role = sql.check_admin(user_id=user.id)
    print(role)
...
def jarvis(update,context):
    ...
    if text.startswith('apolo'):
        ...
        sql.update_group_language(language=language,group_id=group_id,user_id=user_id)
        context.bot.edit_message_text(
            text="The Group was saved as the official {} Group.".format(language),
            chat_id=update.callback_query.message.chat_id,
            message_id=update.callback_query.message.message_id)
    elif text.startswith('apolq'):
        last_id = sql.get_last_apolo_id(user_id)
        group_id = text.split('+')[1]
        sql.change__apolo_qstn_group_id(group_id=group_id, qid=last_id, user_id=user_id)
        apolo_qstn = sql.get_apolo_question(last_id)
        ...
        print(message_id)
        sql.update_apolo_msgid_qstn_type(message_id=message_id,question_type="apolo",tbl_id=last_id,user_id=user_id)
        context.bot.edit_message_text(
        ...
        print(text)
        last_id = sql.get_last_seshat_id(user_id)
        group_id = text.split('+')[1]
        sql.change_seshat_group_id(group_id=group_id, tbl_id=last_id, user_id=user_id)
        # sql.change_qstn_status(status=2, tbl_id=tbl_id, user_id=user_id)
        msg = sql.get_seshat_question(last_id, user_id)
        swali, file_id = msg
        try:
        ...
        message_id = payload.message_id
        print(message_id)
        sql.change_seshat_message_id(message_id=message_id, tbl_id=last_id, user_id=user_id)
        context.bot.edit_message_text(
            text="The Message was successfully posted in the group\nTo post another message click the POST key",
            chat_id=update.callback_query.message.chat_id,
            message_id=update.callback_query.message.message_id)
        ...
        sql.delete_admin(admin_id=admin_id)
        context.bot.edit_message_text(
        ...
        sql.add_zamol_question(user_id=user_id,group_id=group_id,language=language,questiontype='zamol')
        context.bot.edit_message_text(
        ...
        tbl_id = text.split('+')[1]
        lang, qstn,file_id,g_id = sql.get_zamol_question(tbl_id=tbl_id,user_id=user_id)
        ...
        message_id = payload.message_id
        print(message_id)
        sql.change_zamol_qstn_message_id(message_id=message_id,tbl_id=tbl_id,user_id=user_id)
        context.bot.edit_message_text(
        ...
        sql.add_gaia_question(user_id=user_id,group_id=group_id,language=language,questiontype='gaia')
        context.bot.edit_message_text(
            text=f"Please record your message in {language}",
            chat_id=update.callback_query.message.chat_id,
            message_id=update.callback_query.message.message_id)
        ...
        tbl_id = text.split('+')[1]
        lang, qstn,file_id,g_id = sql.get_gaia_question(tbl_id=tbl_id,user_id=user_id)
        payload = context.bot.send_voice(chat_id=g_id, voice=file_id,
                                 caption="🗣Please listen to Ro’s recording, then long-press, click reply and record what you heard!")
        message_id = payload.message_id
        print(message_id)
        sql.change_gaia_qstn_message_id(message_id=message_id,tbl_id=tbl_id,user_id=user_id)
        context.bot.edit_message_text(
            text="The Message was successfully posted in the group\nTo post another message click the POST button",
            chat_id=update.callback_query.message.chat_id,
            message_id=update.callback_query.message.message_id)
...
def observer(update,context):
    ...
    if bot_username==config.BOT_USERNAME and (chat_type=="group" or chat_type=="supergroup"):
        ...
        sql.set_user(user_id=user.id,username=user.username,role=0)
        message_id = update.message.reply_to_message.message_id
        qstn_type = util.question_type(message_id=message_id)
        print(qstn_type)
        if qstn_type=='apolo':
            sql.add_user_answer(user_id=user.id, correct=0, incorrect=0, answertype=qstn_type)
            answer = sql.get_apolo_answer_by_msg_id(msg_id=message_id)
            correct, incorrect = sql.get_correct_incorrect(user_id=user.id,answertype=qstn_type)
            if user_answer.lower()==answer.lower():
                status = sql.get_apolo_qstn_status(msg_id=message_id)
                if status == 0:
                    sql.change_apolo_qstn_status(status=1,msg_id=message_id)

                    # print(correct, incorrect)
                    correct =+1
                    sql.update_correct(correct=correct,answertype=qstn_type,user_id=user.id)
                ...
                print(correct, incorrect)
                incorrect +=1
                sql.update_incorrect(incorrect=incorrect,answertype=qstn_type,user_id=user.id)
                update.message.reply_text("🐔Sorry, your answer is wrong. Please try again!")
        elif qstn_type=='seshat':
            sql.add_user_answer(user_id=user.id, correct=0, incorrect=0, answertype=qstn_type)
            answer = sql.get_seshat_answer_by_msg_id(msg_id=message_id)
            correct, incorrect = sql.get_correct_incorrect(user_id=user.id,answertype=qstn_type)
            if user_answer.lower() == answer.lower():
                status = sql.get_seshat_qstn_status(msg_id=message_id)
                if status == 0:
                    sql.change_seshat_qstn_status(status=1, msg_id=message_id)
                    # print(correct, incorrect)
                    correct = +1
                    sql.update_correct(correct=correct, answertype=qstn_type, user_id=user.id)
                    update.message.reply_text("🏆Your answer is correct!!! You earn 1 point.")
            ...
            sql.add_user_answer(user_id=user.id, correct=0, incorrect=0, answertype=qstn_type)
            correct, incorrect = sql.get_correct_incorrect(user_id=user.id,answertype=qstn_type)
            data = sql.get_zamol_qstnsby_msgid(msg_id=message_id,group_id=group_id)
            if user_answer.lower()==data[3].lower():
                status = sql.get_zamol_qstn_status(msg_id=message_id)
                if status == 0:
                    sql.change_zamol_qstn_status(status=1,msg_id=message_id)
                    print(correct, incorrect)
                    correct = +1
                    sql.update_correct(correct=correct, answertype=qstn_type, user_id=user.id)
                    update.message.reply_text("🏆Your answer is correct!!! You earn 1 point.")
            ...
            try:
                group_id = update.message.chat.id
                sql.add_user_answer(user_id=user.id, correct=0, incorrect=0, answertype=qstn_type)
                print(sql.get_correct_incorrect(user_id=user.id, answertype=qstn_type))
                correct, incorrect = sql.get_correct_incorrect(user_id=user.id, answertype=qstn_type)
                ...
                if length < 10:
                    ...
                    speech.file = new
                    data = sql.get_gaia_qstnsby_msgid(msg_id=message_id, group_id=group_id)
                    ...
                    if text == 401:
                        ...
                        status = sql.get_gaia_qstn_status(msg_id=message_id)
                        print(status)
                        if status == 0:
                            sql.change_gaia_qstn_status(status=1, msg_id=message_id)
                            print(correct, incorrect)
                            correct = +1
                            sql.update_correct(correct=correct, answertype=qstn_type, user_id=user.id)
                            update.message.reply_text("🏆Your answer is correct!!! You earn 1 point.")
